...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', '*Con otro telefono que tengas o en la PC escanea este QR para convertirte en un sub bot*\n\n*1. Haga clic en los tres puntos en la esquina superior derecha*\n*2. Toca WhatsApp Web*\n*3. Escanee este codigo QR*\n*Este código QR expira en 20 segundos!*', m)
      setTimeout(() => {
  ...
  } else throw '*「Bot」*\n\n*No puede ser un bot dentro de un bot!*\n*Mande el comando #serbot al numero oficial/principal del Bot*\n\n*👉🏻 https://wa.me/' + global.conn.user.jid.split`@`[0] + '?text=#serbot*\n\n*「Bot」*'
}
handler.help = ['jadibot']
handler.tags = ['jadibot']

handler.command = /^serbot|jadibot|quieroserbot$/i
handler.prems = false
