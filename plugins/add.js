...
let handler = async (m, { conn, text, participants, usedPrefix }) => {
  ...
  try {
	...
	for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
	  ...
	  await conn.sendGroupV4Invite(m.chat, jid, invite_code, invite_code_exp, false, '_Hola 👋🏻, soy Bot Tiburon V2, un Bot de WhatsApp, un integrante de este grupo intento agregarte, pero no pudo :( es por ello que se te envia esta invitación para que puedas agregarte tu mism@._ *Únete, te esperamos!!*', jpegThumbnail ? {
		jpegThumbnail
	  } : {})
	}
} catch (e) {
